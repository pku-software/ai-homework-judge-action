...
def get_cases() -> List[Union[Case, MalformedCase]]:
    return [
        ...
        Case(AiType.DRAW, "d2", "draw_output2.bin", False),
        Case(AiType.DRAW, "d3", None, True),
        Case(AiType.DRAW, "d4", "draw_output4.bin", True),

        Case(AiType.DRAW, "d1e", None, False, error=True),
        Case(AiType.DRAW, "d2e", "draw_output2e.bin", False, error=True),
        Case(AiType.DRAW, "d3e", None, True, error=True),
        Case(AiType.DRAW, "d4e", "draw_output4e.bin", True, error=True),

        Case(AiType.WOLFRAM, "m1", None, False),
        Case(AiType.WOLFRAM, "m2", "math_output2.bin", False),
        Case(AiType.WOLFRAM, "m3", None, True),
        Case(AiType.WOLFRAM, "m4", "math_output4.bin", True),

        Case(AiType.WOLFRAM, "m1e", None, False, error=True),
        Case(AiType.WOLFRAM, "m2e", "math_output2e.bin", False, error=True),
        Case(AiType.WOLFRAM, "m3e", None, True, error=True),
        Case(AiType.WOLFRAM, "m4e", "math_output4e.bin", True, error=True),
        ...
        MalformedCase(["--chat", "--output"]),
        MalformedCase(["--chat", "--unrecognized"]),
        MalformedCase(["--chat", "p", "--unrecognized"]),
        MalformedCase(["--chat", "p", "--unrecognized", "argument"]),
        ...
        MalformedCase(["--chat", "--draw", "--math", "many_multiple_type"]),
        MalformedCase(["--chat", "duplicate", "--chat", "type"]),
        MalformedCase(["--chat", "--chat", "duplicate_type"]),
        MalformedCase(["--chat", "--chat", "--chat", "many_duplicate_type"]),
        MalformedCase(["--chat", "p", "--output", "duplicate.txt", "-o", "output.txt"]),
    ]
